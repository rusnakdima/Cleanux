...
fn test_provider_clone_sharing() {
  ...
  // Both arcs point to the same data (same pointer)
  assert_eq!(provider1.as_ref(), provider2.as_ref());
  // Arc::clone gives the SAME pointer (shared ownership via reference count)
  assert_eq!(provider1.as_ptr(), provider2.as_ptr());

  // But they are separate Arc instances that both hold the data
  // When one is dropped, the other still has the data
  drop(provider2);
  // provider1 still holds the data
  assert_eq!(provider1.as_ref(), "mock_provider");
}
