use cleanux_lib::utils::response_helper::{
...
fn test_response_builder_new() {
  ...
  assert_eq!(response.status, Status::Info);
}
...
fn test_response_builder_with_data() {
  let response = ResponseBuilder::new()
    .success("OK")
    .data(serde_json::Value::String("test data".to_string()))
    .build();
  assert_eq!(response.status, Status::Success);
  assert_eq!(response.message, "OK");
  match response.data {
    Some(serde_json::Value::String(s)) => assert_eq!(s, "test data"),
...
fn test_success_response() {
  let response = success_response(serde_json::Value::String("data".to_string()), "Success!");
  assert_eq!(response.status, Status::Success);
...
fn test_info_response() {
  let response = info_response(serde_json::Value::Bool(true), "Info");
  assert_eq!(response.status, Status::Success);
...
fn test_error_response() {
  let response = error_response("Error occurred", serde_json::json!(0));
  assert_eq!(response.status, Status::Error);
...
fn test_data_empty_string() {
  ...
  match data {
    serde_json::Value::String(s) => assert_eq!(s, ""),
...
fn test_models_into_data_array_empty() {
  ...
  assert!(result.is_ok());
  match result.unwrap() {
    serde_json::Value::Array(arr) => assert_eq!(arr.len(), 0),
