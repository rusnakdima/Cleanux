//! Application error types extending [`tauri_shared::AppError`] with Cleanux-specific variants.

use serde::{Deserialize, Serialize};
use std::fmt;

/// Unified error type combining standard error variants and Cleanux-specific extensions.
  ...
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Self::PathOutsideAllowed(msg) => {
        write!(f, "Path outside allowed directories: {}", msg)
      }
...
impl AppError {
  ...
  pub fn into_response(self) -> crate::Response<serde_json::Value> {
    ...
    match self {
      ...
      Self::Unknown(msg) => Response::error(msg),
    }
  }
}
