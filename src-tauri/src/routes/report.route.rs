...
pub async fn crud_generate_cleaning_report(
  ...
  state
    ...
    .await
    .map(|doc| ResponseModel::new_success_with_data("Report generated", doc))
...
pub async fn crud_get_cleaning_history(
  ...
  state
    ...
    .await
    .map(|docs| ResponseModel::new_success_with_array("Cleaning history retrieved", docs))
    .map_err(|e| ResponseModel::from(e))
}
...
pub async fn crud_compare_snapshots(
  ...
  let before_doc = state
    ...
    .await
    .map_err(|e| ResponseModel::from(e))?
    .ok_or_else(|| ResponseModel::new_false("Before report not found"))?;
  ...
  let after_doc = state
    ...
    .await
    .map_err(|e| ResponseModel::from(e))?
    .ok_or_else(|| ResponseModel::new_false("After report not found"))?;
  ...
  Ok(ResponseModel::new_success_with_data(
    "Snapshots compared",
