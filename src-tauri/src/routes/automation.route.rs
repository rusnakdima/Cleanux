...
pub async fn crud_get_execution_history(
  ...
  Ok(ResponseModel {
    ...
    message: "Execution history retrieved".to_string(),
    data: DataValue::Array(docs),
  })
}

#[tauri::command]
pub async fn crud_get_quick_actions() -> Result<ResponseModel, ResponseModel> {
  use crate::services::automation_service::AutomationService;

  let actions = AutomationService::get_quick_actions_list();
  Ok(ResponseModel {
    status: ResponseStatus::Success,
    message: "Quick actions retrieved".to_string(),
    data: DataValue::Object(serde_json::to_value(actions).unwrap_or_default()),
  })
}

#[tauri::command]
pub async fn crud_execute_action(action_id: String) -> Result<ResponseModel, ResponseModel> {
  use crate::services::automation_service::AutomationService;
...
pub async fn crud_execute_recipe(
  ...
  let filter =
    nosql_orm::query::Filter::from_json(&filter).map_err(|e| ResponseModel::from(e.to_string()))?;
  ...
  let recipe = recipes.into_iter().next().ok_or_else(|| ResponseModel {
    status: ResponseStatus::Error,
    message: "Recipe not found".to_string(),
    data: DataValue::String(String::new()),
  })?;

  AutomationService::execute_recipe_from_entity(recipe).map_err(|e| ResponseModel::from(e))
}
