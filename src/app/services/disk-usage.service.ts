...
export class DiskUsageService {
  ...
  constructor() {
    this.logger.logInfo('service', 'DiskUsageService', 'init', 'DiskUsageService initialized');
  }

  async scanDirectory(path: string): Promise<ScanResult> {
    this.logger.logInfo('service', 'DiskUsageService', 'scanDirectory', 'Scanning directory', {
      path,
    });
    try {
      ...
      this.logger.logInfo(
        ...
        { path, totalSize: result.totalSize }
      );
      return result;
    } catch (error) {
