...
export class CleanerViewStore {
  ...
  async loadActiveTabData(): Promise<void> {
    ...
    try {
      ...
      this.notification.cleanError(`load ${tab} data`, error);
    } finally {
  ...
  async loadMoreCache(): Promise<void> {
    ...
    try {
      ...
      this.notification.cleanError('load more cache data', error);
    } finally {
      this.loading.set(false);
    }
  }
  ...
  async clearSelectedForActiveTab(): Promise<void> {
    ...
    try {
      ...
      this.notification.cleanError('create backup', error);
    }
    ...
    try {
      ...
      this.notification.error('Failed to clear files: ' + getErrorMessage(error), error);
    } finally {
      this.loading.set(false);
    }
  }
  ...
  async cleanPackageCache(manager: string): Promise<void> {
    ...
    try {
      ...
      this.notification.error('Failed to clean package cache: ' + getErrorMessage(error), error);
    } finally {
      this.loading.set(false);
    }
  }
  ...
  async cleanAllPackageCaches(): Promise<void> {
    ...
    try {
      ...
      this.notification.error('Failed to clean package caches: ' + getErrorMessage(error), error);
    } finally {
      this.loading.set(false);
    }
  }
